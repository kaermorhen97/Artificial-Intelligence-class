function [ calculatedDistance ] = DistanceCalculator(inputCoordinate, from, to)
    % DistanceCalculator This function calculates the distance from the current point to the next point of the path.
    % 
    ...
    % (x, y) coordinates. 
    calculatedDistance = sqrt((inputCoordinate(from, 1) - inputCoordinate(to,1)).^2 + (inputCoordinate(from, 2) - inputCoordinate(to, 2)).^2);
end
